...
export interface NodeCryptoEngineEncryptParams
  extends pkijs.CryptoEngineEncryptParams {
  pbeSchema?: string;
}

export default class NodeCryptoEngine extends pkijs.CryptoEngine {
  constructor() {
    super({
      crypto,
  ...
  public override getRandomValues<T extends ArrayBufferView | null>(
    array: T,
  ): T {
    if (array) {
      const randomView = nodeCrypto.getRandomValues(array.byteLength);
  ...
  public override getAlgorithmByOID(
    oid: string,
    safety?: boolean,
    target?: string,
  ) {
  ...
  public override async encryptEncryptedContentInfo(
    ...
    if (parameters.pbeSchema.toUpperCase() !== 'PBES2') {
      // Assume we have PBES1 here
      ...
      const encryptedContent = await nodeCrypto.encryptUsingPBKDF1Password(
...
*/
  ...
  public override async decryptEncryptedContentInfo(
    ...
    if (pbes1EncryptionAlgorithm.length) {
      ...
      const pbesParameters =
        parameters.encryptedContentInfo.contentEncryptionAlgorithm
          .algorithmParams;

      const saltBuffer = pbesParameters.valueBlock.value[0].valueBlock.valueHex;
      const iterationCount =
        pbesParameters.valueBlock.value[1].valueBlock.valueDec;
      //#endregion

      return nodeCrypto.decryptUsingPBKDF1Password(
  ...
  protected getHashAlgorithmLength(hashAlgorithm: string): number {
    switch (hashAlgorithm.toLowerCase()) {
      ...
      default:
        throw new Error(
          `Incorrect 'parameters.hashAlgorithm' parameter: ${hashAlgorithm}`,
        );
    }
  }
  public override async stampDataWithPassword(
    ...
    return nodeCrypto.stampDataWithPassword(
  ...
  public override async verifyDataStampedWithPassword(
    ...
    return nodeCrypto.verifyDataStampedWithPassword(
      parameters.hashAlgorithm,
      length,
      parameters.password,
      parameters.salt,
